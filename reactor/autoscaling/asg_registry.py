...
For a minion to be auto-accepted by the salt master, there are two separate events that need to happen:

  1. new-instance:
    The autoscaling group must have published that minion will connect.
  2. new-minion:
    The minion must have connected to the master.

When both requirements are satisfied, a minion's key can be accepted.
This script keeps a small sqlite database of the autoscaling states minions are in.

Taken From:
https://github.com/JensRantil/saltstack-autoscaling/blob/master/reactors/autoscaling/autoscale_registry.py
  ...
  with contextlib.closing(lock), sqlite_connection(args.database_file) as conn:
    ...
    for instance in args.instances:
      ...
      if record is None:
        record = Record(
            instanceid=instance,
            instancetimestamp=time.time(),
            miniontimestamp=None)
      else:
        record = record._replace(instancetimestamp=time.time())
      print " ===> Adding new ec2 instance record: {}".format(record)
...
def new_minion(args):
  ...
  with contextlib.closing(lock), sqlite_connection(args.database_file) as conn:
    ...
    for minion in args.minions:
      ...
      print " ===> Adding new minion instance record: {}".format(record)
      write_record(conn, record)
    conn.commit()
  return 0
...
def check(args):
  ...
  with contextlib.closing(lock), sqlite_connection(args.database_file) as conn:
    ...
    with sqlite_cursor(conn) as c:
      ...
      c.execute("SELECT instanceid FROM instances"
                " WHERE miniontimestamp IS NULL"
                " AND instancetimestamp IS NOT NULL")
      for row in c:
  ...
  if found:
    print "The minion is ready to be accepted."
    return 0

  print "The minion is NOT ready to be accepted."
  return 1
